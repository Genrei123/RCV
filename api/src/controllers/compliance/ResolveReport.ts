import { storeReportResolutionOnBlockchain, isSepoliaInitialized } from '../../services/sepoliaBlockchainService';
...
export const resolveReport = async (
  ...
  try {
    ...
    if ((!report.agent || !report.agent.email) && !report.kioskId) {
    ...
    try {
      if (isSepoliaInitialized()) {
        const txResult = await storeReportResolutionOnBlockchain({
          reportId: report._id,
          originalStatus,
        ...
        if (txResult) {
          report.txHash = txResult.txHash;
          report.blockchainVerifiedAt = txResult.timestamp;
          await complianceRepo.save(report);
        }
      }
    } catch (bcError) {
      console.error('Failed to store report resolution on blockchain:', bcError);
      // We don't fail the request if blockchain fails, just log it
    }
    ...
    res.status(200).json({
      ...
      data: {
        ...
        etherscanUrl: report.txHash ? `https://sepolia.etherscan.io/tx/${report.txHash}` : undefined,
      },
    });
  } catch (error: any) {

...
export class ProductService {
    ...
    static async addProduct(productData: CreateProductRequest) {
        try {
            const response = await apiClient.post<CreateProductResponse>('/product/products', productData);
            return response.data;
        } catch (error: any) {
    ...
    static async getProductById(id: string) {
        try {
            const response = await apiClient.get<{ product: Product }>(`/product/products/${id}`);
            return response.data;
        } catch (error: any) {
